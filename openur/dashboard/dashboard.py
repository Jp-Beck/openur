...
class DashboardClient:
    ...
    def connect(self):
        """Establish connection to the dashboard server with retry logic."""
        retries = 0
        while not self.exit_flag.is_set() and retries < self.max_retries:
            try:
                with self.lock:
                    ...
                    self.conn.settimeout(10)  # Add timeout
                ...
                if retries < self.max_retries:
                    time.sleep(min(5 ** retries, 30))  # Cap sleep time at 30 seconds
                if self.conn:
                    self.conn.close()
                    self.conn = None
        return False
    ...
    def close(self):
        """Close the dashboard connection safely."""
        try:
            if self.conn:
                self.conn.close()
    ...
    def is_connected(self):
        """Check if the dashboard connection is active."""
