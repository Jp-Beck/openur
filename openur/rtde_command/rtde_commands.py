...
class RTDECommands:
    ...
    def connect(self):
        """Establish RTDE connection with proper error handling."""
        retries = 0
        while not self.exit_flag.is_set() and retries < self.max_retries:
            try:
                ...
                if retries < self.max_retries:
                    time.sleep(min(5 ** retries, 30))  # Cap sleep time
                if self.con:
                    self.con.disconnect()
                    self.con = None
        return False
    ...
    def is_connected(self):
        """Check if RTDE connection is active."""
    ...
    def speed_slider_mask(self, n: int=0):
        """Get or set speed slider mask."""
        if n in [0, 1]:
            return self.fetch_data('speed_slider_mask')
        else:
            raise ValueError("n must be 0 or 1")

    def speed_slider_fraction(self, value: float = None):
        """Get or set speed slider fraction (0.0 to 1.0)."""
        if value is not None:
            if not 0.0 <= value <= 1.0:
                raise ValueError("value must be between 0.0 and 1.0")
            if not self.con:
                raise ConnectionError("No RTDE connection available")
            with self.lock:
                self.setp.speed_slider_mask = 1
                self.setp.speed_slider_fraction = value
                self.con.send(self.setp)
        return self.fetch_data('speed_slider_fraction')

    def standard_digital_output_mask(self, n: int = None):
        """Get or set standard digital output mask."""
        if n is not None:
            if not 0 <= n < 8:
            ...
            with self.lock:
                ...
                self.con.send(self.setp)
        return self.fetch_data('standard_digital_output_mask')
